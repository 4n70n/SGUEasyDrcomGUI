...
#define EAP_PERFORM(step) EAP_PERFORM_EX(step, pkt_data, handler_success, handler_error)

#define EAP_PERFORM_EX(step, data, success, error)                                                          \
    {                                                                                                       \
        int ret;                                                                                            \
        try {                                                                                               \
            if ((ret = pcap.send(data, success, error)) < 0)                                                \
            {                                                                                               \
...
public:
    pcap_dealer(std::string nic, std::vector<uint8_t> mac)
    {
        ...
        if (NULL == (handle = pcap_open_live(nic.c_str(), SNAP_LEN, 1, 1000, errbuf)))
	...
	int send_alive(std::vector<uint8_t> data, std::vector<uint8_t> *success, std::string *error)
	{
		try
		{
			...
			{
			case 0:
				//send_alive(data, success, error);
				throw easy_drcom_exception("pcap_next_ex: time out.");
			...
			case 1: // Success
				...
				break;
			default:{
					throw easy_drcom_exception("pcap_next_ex: " + std::string(pcap_geterr(handle)));
			}
			}
		}
		catch (std::exception& e)
...
public:
    ...
    int response_md5_challenge(std::vector<uint8_t> gateway_mac)
    {
        ...
        auto handler_success = [&](std::vector<uint8_t> recv) -> int {
            ...
            if (eap_header->eap_code == 0x01) // Request
            {
				...
				else if (!noti.compare("userid error3"))
				{
					ret = 3;
				...
				else if (noti.find("Mac, IP, NASip, PORT err(2)!") != std::string::npos)
				{
					ret = 4;
					EAP_LOG_INFO("Tips: Wrong mac address, check if you're using workplace username." << std::endl);
				}
				else if (noti.find("Mac, IP, NASip, PORT err(11)!") != std::string::npos)
				{
					ret = 5;
					EAP_LOG_INFO("Tips: Wrong mac address, usually occrued when you're using different adapter." << std::endl);
				}
				else if (!noti.compare("In use !"))
				{
					ret = 6;
				}
				else if (!noti.find("Authentication Fail ErrCode=05"))
				{
					ret = 7;
					EAP_LOG_INFO("Tips: The account is suspended.");
				}
                
                logoff(gateway_mac); // Need to send a logoff, or the gateway will always send notification
                
                resp_notification = noti;
                return ret; // Don't retry when notification
            }
        ...
        EAP_PERFORM("Response, MD5-Challenge EAP");
    }

	int alive_identity(std::vector<uint8_t> gateway_mac)
	{
		EAP_LOG_INFO("Reciving Alive Request From Gateway..." << std::endl);
		...
		std::string error;
		int count = 0;
		int ret = pcap.send_alive(pkt_data, &success, &error);

		if (ret)
		{
			if (ret != 0){
				EAP_LOG_ERR("Send 802.1X Alive Packet Err, Wait 1s Retry." << std::endl);
				count++;
				if (count > 8){
					EAP_LOG_ERR("Found NO EAP Alive Request, Maybe it's not the time. We'll try a little later." << std::endl);
					return 0;
				}
				Sleep(1000);
				alive_identity(gateway_mac);
			}
			else{
				EAP_LOG_INFO("Recived and Sent 802.1X Alive Pack." << std::endl);
				return 0;
			}
		}
	}
    ...
    std::string getNotification() const { return resp_notification; }
