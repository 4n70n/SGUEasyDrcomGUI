...
public:
	udp_dealer(std::string gateway_ip, uint32_t gateway_port, std::string local_ip) : sock(-1)
    {
        ...
        local.sin_port = htons(gateway_port); // system defined
	...
	int receive(std::function<int(std::vector<uint8_t>)> success, std::function<void(std::string)> error = nullptr)
	{
		std::vector<uint8_t> recv;
...
#define U64_PERFORM(step) U64_PERFORM_EX(step, pkt_data, handler_success, handler_error)

#define U64_PERFORM_EX(step, data, success, error)                                                      \
{                                                                                                       \
    int ret;                                                                                            \
    try {                                                                                               \
        if ((ret = udp.post(data, success, error)) < 0)                                                 \
        {                                                                                               \
            U64_LOG_ERR("Failed to perfrom " << step << ", stopped." << std::endl);                     \
            return -1;                                                                                  \
        }                                                                                               \
    } catch (std::exception &e) {                                                                       \
        U64_LOG_ERR(step << ": " << e.what() << std::endl);                                             \
    }                                                                                                   \
    return ret;                                                                                         \
}

#define U64_HANDLE_ERROR(step)                                                                      \
    auto handler_error = [&](std::string error) {                                                   \
        U64_LOG_ERR(step << ": " << error << std::endl)                                             \
    };

#define U64_LOG_RECV_DUMP(step)                                                                     \
{                                                                                                   \
    U64_LOG_DBG("Received after " << step << ", dump:" << std::endl);                                \
    hexdump(recv);                                                                                  \
}

#define U64_LOG_SEND_DUMP                                                                           \
{                                                                                                   \
    U64_LOG_DBG("send packet data dump:" << std::endl);                                             \
    hexdump(pkt_data);                                                                              \
}

#define U64_RECEIVE(step) U64_RECEIVE_EX(step, handler_success, handler_error)

#define U64_RECEIVE_EX(step, success, error)															\
{                                                                                                       \
	int ret;                                                                                            \
	try {																								\
		if ((ret = udp.receive(success, error)) < 0)					                                \
		{                                                                                               \
			U64_LOG_ERR("Failed to receive " << step << ", stopped." << std::endl);                     \
			return -1;                                                                                  \
		}                                                                                               \
	}	catch (std::exception &e) {																		\
		U64_LOG_ERR(step << ": " << e.what() << std::endl);                                             \
