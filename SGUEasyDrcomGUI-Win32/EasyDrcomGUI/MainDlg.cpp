...
LRESULT CMainDlg::OnInitDialog(UINT /*uMsg*/, WPARAM /*wParam*/, LPARAM /*lParam*/, BOOL& /*bHandled*/)
{
	...
	m_szLogPath = m_szExeDir + _T("\\AnotherDrcom.log");
	m_szConfPath = m_szExeDir + _T("\\AnotherDrcom.conf");
	...
	CString version;
	version.Format(_T("Another Drcom for WIN32 v%d.%d"), HIWORD(lpFfi->dwFileVersionMS), LOWORD(lpFfi->dwFileVersionMS));

	SYS_LOG_INFO(std::string(CT2CA(version)) << std::endl);
	SYS_LOG_INFO("Modified by Yingkit Tse, build on " __DATE__ " " __TIME__ << std::endl);
	...
	if (m_mapNIC.empty()) // no nic found
	{
		...
		{
			MessageBox(_T("ADGUI未能在您的计算机上找到任何网卡。\n请检查网卡是否被禁用！"), _T("获取网卡信息失败！"), MB_ICONERROR);
			SYS_LOG_DBG("ADGUI未能在您的计算机上找到任何网卡。请检查网卡是否被禁用！" << std::endl);
		}
	...
	// set build info
	m_lblBuild.SetWindowText(CString("Modified by Yingkit Tse, build on " __DATE__ " " __TIME__));
	...
	SYS_LOG_INFO("Another Drcom is ready." << std::endl);
	return TRUE;
}
...
{
	MessageBox(_T("特别感谢Shindo的付出，本软件根据EasyDrcom重构\n目前仅实现部分DrCOM v0.8(U64.R110908)的部分功能。"), _T("AnotherDrcom CREDIT"), MB_ICONINFORMATION);
	ShellExecute(NULL, _T("open"), _T("https://github.com/yingkittse"), NULL, NULL, SW_SHOWNORMAL);
	return 0;
}
...
{
	SYS_LOG_INFO("Another Drcom GUI quit." << std::endl);
...
LRESULT CMainDlg::OnPopupExitClicked(WORD /*wNotifyCode*/, WORD /*wID*/, HWND /*hWndCtl*/, BOOL& /*bHandled*/)
{
	if (m_thConnectJob)
	{
		if (IDNO == MessageBox(_T("一旦退出Another Drcom GUI，您将会在稍后失去网络连接。"), _T("确定要退出吗？"), MB_ICONEXCLAMATION | MB_YESNO))
			return 0;
	}
...
DWORD CMainDlg::CThreadStuDistKeepAlive::Run()
{
	...
	eap_dealer *alive_dealer = nullptr;
...
#define CSTRING_TO_STD(x) std::string(CT2CA(x))
	try {
		alive_dealer = new eap_dealer(CSTRING_TO_STD(dlg->m_szStoredNIC), str_mac_to_vec(CSTRING_TO_STD(dlg->m_szStoredMAC)),
...
#undef CSTRING_TO_STD
	...
	try 
	{
		...
		if (alive_dealer->alive_identity(broadcast_mac) == 0){
			alive_try = 0;
		}
		else{
			alive_try++;
...
#define CSTRING_TO_STD(x) std::string(CT2CA(x))
		...
		try {
			udp = new drcom_dealer_u62(str_mac_to_vec(CSTRING_TO_STD(dlg->m_szStoredMAC)), CSTRING_TO_STD(dlg->m_szStoredIP),
				CSTRING_TO_STD(dlg->m_szStoredUserName), CSTRING_TO_STD(dlg->m_szStoredPassWord), "192.168.127.129", 61440, "Kit-Laptop", "DrCOM x");
		}
		catch (std::exception& ex) {
...
{
	bool firstTry = true;
	drcom_dealer_u64 *dealer = reinterpret_cast<drcom_dealer_u64*>(param);
	eap_dealer *alive_dealer = nullptr;
...
#undef CSTRING_TO_STD
	...
	{
		// first try
		dlg->m_lblStatus.SetWindowText(_T("UDP登陆中……"));
		if (dealer->start_request()) goto udp_fail;
		if (dealer->send_login_auth() == 0){
			count = 0;
		}
		//if (dealer->recv_svr_message()) goto udp_fail;
...
#define CSTRING_TO_STD(x) std::string(CT2CA(x))
		...
		try {
			udp = new drcom_dealer_u64(str_mac_to_vec(CSTRING_TO_STD(dlg->m_szStoredMAC)), CSTRING_TO_STD(dlg->m_szStoredIP),
				CSTRING_TO_STD(dlg->m_szStoredUserName), CSTRING_TO_STD(dlg->m_szStoredPassWord), "192.168.127.129", 61440, "Kit-Laptop", "DrCOM x");
		}
		catch (std::exception& ex) {
			U62_LOG_ERR(ex.what() << std::endl);
...
#undef CSTRING_TO_STD
	...
	dlg->m_lblStatus.SetWindowText(_T("802.1X注销中……"));
	if (eap) eap->logoff(nearest_mac);
	//if (eap) eap->logoff(broadcast_mac);
