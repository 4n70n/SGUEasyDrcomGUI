...
- (void)applicationDidFinishLaunching:(NSNotification *)aNotification
{
    ...
    [self.lblBuild setStringValue: [NSString stringWithFormat:@"Modified by 4n70n, build on %s %s.", __DATE__, __TIME__]];
    
    SYS_LOG_INFO("EasyDrcomGUI For OSX, v" << [version UTF8String] << std::endl);
    SYS_LOG_INFO("Modified by 4n70n, build on " __DATE__ " " __TIME__ << std::endl);
...
- (IBAction) supportClicked:(id)sender {
    NSWorkspace *workspace = [NSWorkspace sharedWorkspace];
    [workspace openURL:[NSURL URLWithString:@"https://github.com/4n70n"]];
}
